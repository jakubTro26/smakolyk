...
<div id="reviews" class="woocommerce-Reviews">
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
		<?php 
			$review_class = ( class_exists( 'VI_Woo_Photo_Reviews' ) ) ? 'form-photo-reviews' : '';	
		?>
		<div id="review_form_wrapper" class="review_form_wrapper <?php echo esc_attr($review_class); ?>">
			<div id="review_form">
				<?php
				$commenter    = wp_get_current_commenter();
				$comment_form = array(
					/* translators: %s is product title */
					'title_reply'         => have_comments() ? esc_html__( 'Add a review', 'greenmart' ) : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'greenmart' ), get_the_title() ),
					/* translators: %s is product title */
				...
				$fields              = array(
					...
					'email'  => array(
						'label'    => esc_html__( 'Email', 'greenmart' ),
						'type'     => 'email',
				...
				$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . esc_html__( 'Your review', 'greenmart' ) . '&nbsp;<span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" required></textarea></p>';
	...
	<?php else : ?>
		<p class="woocommerce-verification-required"><?php esc_html_e( 'Only logged in customers who have purchased this product may leave a review.', 'greenmart' ); ?></p>
	<?php endif; ?>
