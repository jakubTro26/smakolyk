...
class Tbay_Widget_Recent_Comment extends Tbay_Widget {
    public function __construct() {
        parent::__construct(
            'tbay_recent_comment',
            esc_html__('Tbay Recent Comments Widget', 'greenmart'),
            array( 'description' => esc_html__( 'Show list of recent comments', 'greenmart' ), )
        );
    ...
    public function form( $instance ) {
        if ( isset( $instance[ 'title' ] ) ) {
            ...
            $title = esc_html__( 'Latest Post', 'greenmart' );
        }
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'number_comment' )); ?>"><?php esc_html_e( 'Num Comments:', 'greenmart' ); ?></label>
