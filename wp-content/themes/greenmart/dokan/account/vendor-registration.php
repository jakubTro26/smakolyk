...
<form id="dokan-vendor-register" method="post" class="register dokan-vendor-register">
    ...
    <div class="split-row name-field form-row-wide">
        <p class="form-row form-group">
            <label for="first-name"><?php esc_html_e( 'First Name', 'greenmart' ); ?> <span class="required">*</span></label>
        ...
        <p class="form-row form-group">
            <label for="last-name"><?php esc_html_e( 'Last Name', 'greenmart' ); ?> <span class="required">*</span></label>
    ...
    <p class="form-row form-group form-row-wide">
        <label for="shop-phone"><?php esc_html_e( 'Phone Number', 'greenmart' ); ?><span class="required">*</span></label>
    ...
    <?php if( greenmart_tbay_get_config('show_confirm_password', true) ) : ?>
    <p class="form-row form-row-wide">
        <label for="reg_password"><?php esc_html_e( 'Confirm Password', 'greenmart' ); ?> <span class="required">*</span></label>
        <input type="password" class="input-text form-control" placeholder="<?php esc_attr_e('Confirm Password', 'greenmart'); ?>" name="password2" id="reg_password2" value="<?php if ( ! empty( $_POST['password2'] ) ) echo esc_attr( $_POST['password2'] ); ?>" />
    </p>
    <?php endif; ?>

    <!-- Spam Trap -->
    <div style="left:-999em; position:absolute;"><label for="trap"><?php esc_html_e( 'Anti-spam', 'greenmart' ); ?></label><input type="text" name="email_2" id="trap" tabindex="-1" /></div>

    <p class="form-row form-group form-row-wide">
        <label for="company-name"><?php esc_html_e( 'Shop Name', 'greenmart' ); ?> <span class="required">*</span></label>
    ...
    <p class="form-row form-group form-row-wide">
        <label for="seller-url" class="pull-left"><?php esc_html_e( 'Shop URL', 'greenmart' ); ?> <span class="required">*</span></label>
    ...
    if ( 'on' === $show_terms_condition && $terms_condition_url ) { ?>
        <p class="form-row form-group form-row-wide">
            ...
            <label style="display: inline" for="tc_agree"><?php echo sprintf( __( 'I have read and agree to the <a target="_blank" href="%s">Terms &amp; Conditions</a>.', 'greenmart' ), esc_url( $terms_condition_url ) ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped  ?></label>
        </p>
    <?php }
    ...
    <p class="form-row">
        ...
        <input type="submit" class="dokan-btn dokan-btn-theme" name="register" value="<?php esc_attr_e( 'Register', 'greenmart' ); ?>" />
    </p>
