...
( function( api ) {
	var cssTemplate = wp.template( 'greenmart-color-scheme' ),
		colorSchemeKeys = [
			'background_color',
	...
	api.controlConstructor.select = api.Control.extend( {
		ready: function() {
			if ( 'color_scheme' === this.id ) {
				this.setting.on( 'change', function( value ) {
	...
	function updateCSS() {
		...
		colors.border_greenmart_color = Color( colors.textcolor ).toCSS( 'rgba', 0.3 );
		...
		colors.sidebar_border_greenmart_color = Color( colors.sidebar_textcolor ).toCSS( 'rgba', 0.3 );
